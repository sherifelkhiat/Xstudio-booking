            ...
            <template v-else>
                <div class="grid grid-cols-[1fr_auto] gap-8 max-lg:grid-cols-[1fr] max-md:gap-5">
                    ...
                    <div class="sticky top-8 block h-max w-[442px] max-w-full max-lg:w-auto max-lg:max-w-[442px] ltr:pl-8 max-lg:ltr:pl-0 rtl:pr-8 max-lg:rtl:pr-0">
                        ...
                        <label>
                            <input type="checkbox" v-model="payWithDeposit" @change="handlePayWithDeposit"> @lang('xbooking::app.shop.pay_with_deposite')
                        </label>

                        <!-- Conditionally show the deposit details -->
                        <div v-if="payWithDeposit">
                            @if($cart->items->isNotEmpty())
                                @php
                                    $firstItem = $cart->items->first();
                                    $product = $firstItem->product;

                                    // Retrieve all custom attributes that are visible on the front
                                    $customAttributes = $product->attribute_family
                                                                ->custom_attributes()
                                                                ->get();

                                    // Find the 'deposite' attribute from custom attributes
                                    $depositeAttribute = $customAttributes->where('code', 'deposit')->first();

                                    // Get the value of 'deposite' if it exists
                                    $deposite = $depositeAttribute ? $product->{$depositeAttribute->code} : 0;
                                    
                                @endphp

                                <p>@lang('xbooking::app.shop.deposite'): {{ $deposite }}</p>
                            @else
        ...
        <script type="module">
            app.component('v-checkout', {
                ...
                methods: {
                    getCart() {
                        this.$axios.get("{{ route('shop.checkout.onepage.summary') }}")
                            .then(response => {
                                ...
                                if (this.payWithDeposit) {
                                    this.cart.formatted_grand_total  = '{{ $deposite }}'
                                    console.log(this.cart.grand_total)
                                }
                    ...
                    handlePayWithDeposit() {
                        if (this.payWithDeposit) {
                            this.cart.formatted_grand_total  = '{{ $deposite }}'
                            console.log(this.cart.grand_total)
                        } else {
                            this.cart.formatted_grand_total = '{{$cart->grand_total}}'
                        }
                    },
                    ...
                    placeOrder() {
                        ...
                        const payload = {
                            payWithDeposit: this.payWithDeposit,
                            deposite: '{{ $deposite }}'
                        };

                        this.$axios.post('{{ route('shop.xbooking.place.order') }}', payload)
                            .then(response => {
